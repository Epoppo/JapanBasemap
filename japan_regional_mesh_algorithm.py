...
class NamingRule(Flag):
    ...
    ルール詳細
    FIRST_MESH: 緯度*1.5(2桁) + 経度-100(2桁)
    DIVIDE_XPARTS = 親のメッシュ1つを縦にm,横にn個に分割した際、0スタートで南西から数えてo,p番目(2桁)
    DIVIDE_SPECIAL_4PARTS = 親のメッシュ十字に分割した際、南西を0, 南東を1, 北西を2, 北東を3(1桁)
    TWO_TIME_MESH = 親のメッシュ1つを縦に5,横に5個に分割した際、0スタートで南西から数えてo*2,p*2番目(2桁)と、末尾に"5"を加える(1桁)
...
class RectData(NamedTuple):
    """緯度経度(10進法における度) を管理、格納しておくためのデータ型
    これ自体は四隅以上の意味はないので、変数名の方でしっかり用途を記載しておくこと。
    TIPS: 命名としてはRectDataよりBoundingBoxDataのほうがより適当ではないか？
    """
...
# NamedTupleを使うなら辞書型を使う理由が無い気がしたので
class CrsProperty(NamedTuple):
    """ウィジェット上で表示する説明文と、CRSを保持する要素

    DESCRIPTION(str): ウィジェット上で選択する際の説明文
    EPSG(int): 処理に使用するEPSG。CRS(QgsCoordinationReferenceSystem)を定義する際に使用
...
class MeshProperty(NamedTuple):
    """ウィジェット上で表示する説明文と、メッシュ作成時に使用するデータの要素

    DESCRIPTION(str): ウィジェット上で選択する際の説明文
    parent(Optional[MeshPattern]): 当該プロパティに紐づくMeshPatternにおいて、分割元となるMeshPetternを記載。存在しない場合はNone
    divide(Tuple[int, int]): 親のメッシュを(lat分割数, lon分割数)の「個数」に分割しているかを記載
    naming_rule(NamingRule): 分割手法。該当する内容をNamingRuleクラス内から選んで記載
    figure_sec_size(Optional[Tuple[Union[int, float], Union[int, float]]]): 分割元となる"秒数"を記載。基本的に一番の親になるプロパティでのみ記載し、そうでない場合はNone。

    ex.) 新規に3次メッシュ1/20細分図郭(50mメッシュ)を作成する場合
    1. MeshPatternに新しく要素を追加
    2. _MESH_PROPERTYに新しく追加した要素を以て下記を記載
    まで。
    現状に無い特殊な分割を行う場合は、NamingruleとderiveFigureNameFromLatLonSecに追記を行う。
    """

    DESCRIPTION: str
    PARENT: Optional[MeshPattern]
    DIVIDE: Tuple[int, int]
    NAMING_RULE: NamingRule
    FIGURE_SEC_SIZE: Optional[Tuple[Union[int, float], Union[int, float]]]
...
class JapanRegionalMeshAlgorithm(QgsProcessingAlgorithm):
    ...
    # CRSとデータ表示用プロパティ
    _CRS_PROPERTY = {
        CrsPattern.JGD2011: CrsProperty(
            DESCRIPTION="世界測地系 JGD2011(EPSG:6668)",
            EPSG=6668,
        ),
        CrsPattern.JGD2000: CrsProperty(
            DESCRIPTION="世界測地系 JGD2000(EPSG:4612)",
            EPSG=4612,
        ),
        CrsPattern.TOKYO_DATUM: CrsProperty(
    ...
    _MESH_PROPERTY = {
        MeshPattern.FIRST_MESH: MeshProperty(
        ...
        MeshPattern.FOURTH_MESH: MeshProperty(
        ...
        MeshPattern.FIFTH_MESH: MeshProperty(
        ...
        MeshPattern.SIXTH_MESH: MeshProperty(
        ...
        MeshPattern.TWO_TIMES_MESH_BASED_ON_THIRD_MESH: MeshProperty(
        ...
        MeshPattern.FIVE_TIMES_MESH_BASED_ON_THIRD_MESH: MeshProperty(
        ...
        MeshPattern.ONE_TENTH_MESH_BASED_ON_THIRD_MESH: MeshProperty(
    ...
    def __init__(self):
        super().__init__()
        # 処理内容の選択肢
        self.MESH_CRS = [self.tr(f"{self._CRS_PROPERTY[pattern].DESCRIPTION}") for pattern in CrsPattern]
        self.MESH_TYPE = [self.tr(f"{self._MESH_PROPERTY[pattern].DESCRIPTION}") for pattern in MeshPattern]
    ...
    def initAlgorithm(self, config) -> None:
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                ...
                defaultValue=CrsPattern.JGD2000.value,
                optional=False,
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                ...
                defaultValue=MeshPattern.THIRD_MESH.value,
                optional=False,
            )
        )
    ...
    def processAlgorithm(
        ...
        """
        # 入力された内容の読み込み
        source_crs_property = CrsPattern(self.parameterAsEnum(parameters, self.INPUT, context))  # type: CrsPattern
        ...
        for current, feature in enumerate(features):
            ...
            # 図郭名称の設定
            # 作成したフィーチャの中心点にあたる緯度経度から、フィーチャに命名を行う
    ...
    def fromGansoFigureSecSize(self, mesh_pattern: MeshPattern) -> Tuple[Union[int, float], Union[int, float]]:
        ...
        Return:
            ganso.FIGURE_SEC_SIZE(Tuple[Union[int, float], Union[int, float]]): 祖となる地域メッシュ種別の1メッシュの緯度,経度の大きさを秒で表したもの
        """
        ancestors = self.retrieveMeshAncestors(mesh_pattern)
        ganso = self._MESH_PROPERTY[ancestors[0]]
        return tuple(ganso.FIGURE_SEC_SIZE)
